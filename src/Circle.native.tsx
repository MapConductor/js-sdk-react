...
function CircleWithState({ state }: CircleStateProps): null {
  ...
  useEffect(
    () => () => {
      circleCollector.remove(state.id);
    },
...
export function Circle(props: CircleProps): React.ReactElement | null {
  if (isCirclePositionProps(props)) return <CircleFromPositionProps {...props} />;
...
function CircleFromPositionProps(props: CirclePositionProps): React.ReactElement | null {
  ...
  useEffect(() => {
    state.radiusMeters = props.radiusMeters;
  }, [state, props.radiusMeters]);
  useEffect(() => {
    state.strokeColor = props.strokeColor ?? '#FF0000';
  }, [state, props.strokeColor]);
  useEffect(() => {
    state.strokeWidth = props.strokeWidth ?? 2;
  }, [state, props.strokeWidth]);
  useEffect(() => {
    state.fillColor = props.fillColor ?? 'rgba(255,255,255,0.5)';
  }, [state, props.fillColor]);
  useEffect(() => {
    state.zIndex = props.zIndex ?? null;
  }, [state, props.zIndex]);
  useEffect(() => {
    state.geodesic = props.geodesic ?? true;
  }, [state, props.geodesic]);
  useEffect(() => {
    state.clickable = props.clickable ?? true;
