...
export function Marker({ state }: MarkerStateProps): null {
    ...
    useEffect(() => {
        return () => { markerCollector.remove(state.id); };
...
function syncMarkerState(target: MarkerState, source: MarkerState): void {
    ...
    if (target.onAnimateStart !== source.onAnimateStart) target.onAnimateStart = source.onAnimateStart;
...
export function MarkerFromProps(props: MarkerPropsExpanded): React.ReactElement | null {
    ...
    useEffect(() => { state.onDragEnd = props.onDragEnd ?? null; }, [state, props.onDragEnd]);
