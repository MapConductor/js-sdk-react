...
export function Markers({ states }: MarkersProps): null {
  ...
  useEffect(() => {
    markerCollector.batchChanges(() => {
      ...
      // Remove only the ids this <Markers> instance owns. Using the additive
      // applyDiff() instead of replaceAll()/clear() lets several
      // <Markers>/<Marker> share one map's collector without wiping each
      // other's markers — replaceAll() clears the whole collection, so the last
      // <Markers> to run used to win and erase the rest.
