/**
 * React Native の全プロバイダで共有するブリッジ用コントローラ。
 *
 * RN のコントローラは「JS の状態をネイティブへ送り、ネイティブのイベントを
 * コアへ戻す」だけで、地図エンジンには一切触れない。したがって処理は
 * プロバイダに依らず完全に同一で、違うのはネイティブビューの ref 型だけ。
 * かつては 4 プロバイダがこのファイルの写しを各自持っており、
 * 直すたびに 4 箇所を直す必要があった。
 *
 * プロバイダ側はこれを継承（または型エイリアス）するだけでよい:
 *
 * ```ts
 * export class FooViewController extends ReactNativeBridgeMapViewController<FooViewRef> {}
 * ```
 */
...
import { ReactNativeMapViewHolder } from './ReactNativeMapViewHolder';
import { markerStateToNative } from './markerStateToNative';
import {
  createNativeMarkerIconRegistry,
  encodeMarkerBatch,
  NATIVE_MARKER_BATCH_SIZE,
} from '../marker/NativeMarkerBatch';

/**
 * ネイティブビューの ref。`requireNativeComponent` が返すホストコンポーネントの
 * ref はすべてこの形なので、プロバイダ固有の型エイリアスはこれを満たす。
 * `findNodeHandle` に渡せることを型で保証するために制約として使う。
 */
// eslint-disable-next-line typescript/no-explicit-any
export type NativeViewRef = React.ComponentRef<HostComponent<any>> & NativeMethods;

export class ReactNativeBridgeMapViewController<ViewRef extends NativeViewRef>
...
{
  readonly holder: ReactNativeMapViewHolder<ViewRef>;
  ...
  constructor(
    private readonly nativeRef: React.RefObject<ViewRef | null>,
    cameraPosition: MapCameraPosition
  ) {
    super();
    this.cameraPosition = cameraPosition;
    this.holder = new ReactNativeMapViewHolder<ViewRef>(nativeRef);
  }
